...
function Virtual_host {
    if [ ! -f "/etc/apache2/sites-available/vitrine-dataheberg-home.conf" ]; then
        echo -e "<VirtualHost *:80>\nDocumentRoot /var/www/dataheberg.home/vitrine\nServerName www.dataheberg.home\n</VirtualHost>" > /etc/apache2/sites-available/vitrine-dataheberg-home.conf
        a2ensite vitrine-dataheberg-home
        systemctl reload apache2
    else
        echo "Le fichier vitrine-dataheberg-home est deja cree !!! "
    fi
}
...
function update {
    echo "Installation/Update du Site Vitrine "
    sleep 2
    if [ ! -d "/var/www/dataheberg.home/" ]; then 
        mkdir -p /var/www/dataheberg.home/vitrine;
    fi
    rm -rf /var/www/dataheberg.home/vitrine
    cd /var/www/dataheberg.home
    git clone https://dataheberg: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@github.com/Dataheberg/vitrine.git
    systemctl reload apache2
}
